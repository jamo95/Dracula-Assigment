...
// What are the specified player's next possible moves
LocationID *whereCanDracGo(HunterView currentView, int *numLocations,
...
// give the player trail in round round
void lastTrail(HunterView currentView, PlayerID player, Round round, LocationID trail[TRAIL_SIZE]) {
    assert(currentView != NULL && currentView->view != NULL);
    
    int i;
    int firstMove = player * CHARS_PER_TURN;    //index of first move
    int lastMove;
    if (round == 0) lastMove = 0;
    else lastMove = (round-1) * CHARS_PER_ROUND + firstMove;

    initialiseTrail(trail);
    for (i = firstMove; i <= lastMove; i += CHARS_PER_ROUND) {
        updatePlayerTrail(trail, &currentView->view->pastPlays[i], player);
    }
}
