...
LocationID portCity[NUM_PORT_CITY] = {1, 2, 3, 5, 6, 11, 15, 16, 20, 21, 
                                      22, 27, 29, 31, 35, 37, 38, 39, 42, 
                                      46, 47, 51, 53, 54, 55, 61, 65, 66, 67};
LocationID topArea[NUM_TOP_AREA] = {4, 21, 22, 23, 27, 33, 38, 39, 41, 48, 61};
LocationID leftArea[NUM_LEFT_AREA] = {1, 4, 5, 7, 11, 15, 30, 37, 40, 43, 
                                      55, 56, 63};
LocationID middleArea[NUM_MIDDLE_AREA] = {2, 9, 11, 12, 18, 19, 23, 24, 25, 28,
                                          29, 31, 35, 36, 42, 44, 45, 46, 48, 49, 
                                          50, 52, 60, 63, 67, 68, 70};
LocationID rightArea[NUM_RIGHT_AREA] = {3, 8, 10, 13, 14, 17, 20, 26, 32, 34, 
                                        54, 57, 58, 59, 62, 65, 66, 68, 69};
LocationID bottomArea[NUM_BOTTOM_AREA] = {0, 3, 6, 16, 24, 29, 32, 42, 43, 47, 
                                          53, 54, 64, 65, 67};

// register the best play to the game engine
void submitPlay(DracView gameState, LocationID *adjLoc, Map map, Map railMap, 
                LocationID *path, LocationID best, int isHide, LocationID trail[TRAIL_SIZE],
...
int *knownByHunter(DracView gameState, int *numLoc, LocationID trail[TRAIL_SIZE],
                   LocationID hTrail[NUM_HUNTERS][TRAIL_SIZE], Round currRound);
...
PlayerID mostIsolated(Map map, Map railMap, LocationID *path, Round currRound, 
...
int numOfPossHunter(DracView gameState, Map map, Map railMap, LocationID dest, int *knownLoc,
                    ...
                    LocationID hTrail[NUM_HUNTERS][TRAIL_SIZE], int hHealth[NUM_HUNTERS], 
                    LocationID *path);
...
int isFarAwayEnough(Map map, Map railMap, LocationID *adjLoc, int numLoc, LocationID *path, 
                    Round currRound, LocationID hTrail[NUM_HUNTERS][TRAIL_SIZE]);

// check if the next adjLoc would result in forming a loop
int isLoop(LocationID dest, LocationID trail[TRAIL_SIZE], LocationID hideTrail[TRAIL_SIZE]);
...
int isSurrounded(Map map, Map railMap, LocationID *adjLoc, int numLoc, Round currRound,
...
int dracPathLength(Map map, LocationID src, LocationID dest, LocationID trail[TRAIL_SIZE], 
                   LocationID hideTrail[TRAIL_SIZE], LocationID *nextLoc, int hideAllow);
...
int furthestLength(DracView gameState, Map map, Map railMap, LocationID *adjLoc, int numLoc, 
                   Round round, LocationID *path, LocationID hTrail[NUM_HUNTERS][TRAIL_SIZE]);
...
LocationID predictDracMove(DracView gameState, Map map, Map railMap, LocationID dracSrc, 
                           LocationID dracLoc, Round round, Round currRound, 
                           LocationID hTrail[NUM_HUNTERS][TRAIL_SIZE], LocationID *path);
...
int shouldGoToCD(Map map, Map railMap, LocationID dCurrLoc, LocationID *adjLoc, int numLoc, 
                 Round currRound, LocationID *path, LocationID hTrail[NUM_HUNTERS][TRAIL_SIZE], 
                 int distCurrFromHunt, LocationID trail[TRAIL_SIZE], 
                 LocationID hideTrail[TRAIL_SIZE]);

// check if dracula should go to a place which will cause a teleport in next round
int shouldTeleport(Map map, Map railMap, LocationID hTrail[NUM_HUNTERS][TRAIL_SIZE], 
...
int isDangerous(Map map, Map railMap, int *knownLoc, int knownNumLoc,
                int distFromSea, int distFromHunt, LocationID *adjLoc, int numLoc,
...
int distFromNearestSea(Map map, LocationID location, LocationID trail[TRAIL_SIZE], 
                       LocationID hideTrail[TRAIL_SIZE]);
...
LocationID escapeToSea(DracView gameState, Map map, Map railMap, LocationID *adjLoc, int numLoc,
                       ...
                       int hHealth[NUM_HUNTERS], LocationID trail[TRAIL_SIZE], 
                       LocationID hideTrail[TRAIL_SIZE], LocationID tmpTrail[TRAIL_SIZE], 
                       LocationID tmpHideTrail[TRAIL_SIZE], LocationID *path);
...
LocationID escapeToCity(DracView gameState, Map map, Map railMap, LocationID *adjLoc, int numLoc,
                        ...
                        int hHealth[NUM_HUNTERS], LocationID trail[TRAIL_SIZE], 
                        LocationID hideTrail[TRAIL_SIZE], LocationID tmpTrail[TRAIL_SIZE], 
                        LocationID tmpHideTrail[TRAIL_SIZE], LocationID *path, int area, 
                        int distAdjust);

// find a suitable path to escape to an empty area through sea and 
//  return the next location in that path
LocationID escapeThroughSea(DracView gameState, Map map, Map railMap, LocationID *adjLoc, int numLoc,
                            ...
                            int health, LocationID hTrail[NUM_HUNTERS][TRAIL_SIZE], 
                            int hHealth[NUM_HUNTERS], LocationID trail[TRAIL_SIZE], 
                            LocationID hideTrail[TRAIL_SIZE], LocationID *path, int area);
...
LocationID escapeToEmptyArea(DracView gameState, Map map, Map railMap, LocationID *adjLoc, int numLoc,
                             ...
                             int health, LocationID hTrail[NUM_HUNTERS][TRAIL_SIZE], 
                             int hHealth[NUM_HUNTERS], LocationID trail[TRAIL_SIZE], 
                             LocationID hideTrail[TRAIL_SIZE], LocationID *path, 
                             int area, int distAdjust);
...
int stayToLeaveTrap(DracView gameState, Map map, Map railMap, LocationID dest, int numPossHunt, 
                    Round currRound, int *knownLoc, int knownNumLoc, 
                    LocationID hTrail[NUM_HUNTERS][TRAIL_SIZE], int hHealth[NUM_HUNTERS], 
...
void decideDraculaMove(DracView gameState)
{
    ...
    int *knownLoc = knownByHunter(gameState, &knownNumLoc, trail, hTrail, currRound);
    ...
    if (currRound == 0) {
        ...
        // if dracula has only 2 bloods left
        if (health <= 2) {
        ...
        if (isDangerous(map ,railMap, knownLoc, knownNumLoc, distCurrFromSea, distCurrFromHunt, adjLoc, numLoc, dCurrLoc, hTrail, currRound, path)) {
...
//        }
        ...
        for (i = 0; i < numLoc; i++) {
            ...
            int loop = isLoop(adjLoc[i], trail, hideTrail);
            optimalScore = optimalLocScore(distFromHunt, nNextAdj, possHunt, numHunt,
                                           numPossHunt, locScore, tuneScore, distFromSea, distAdjust, loop);
...
int *knownByHunter(DracView gameState, int *numLoc, LocationID trail[TRAIL_SIZE],
    ...
    *numLoc = 0;
    ...
    if (isResearch(gameState) && knownLoc[0] != 5 && currRound >= 6) {
        shiftRight(knownLoc, 0, *numLoc);
        (*numLoc)++;
        knownLoc[0] = 5;
    }
...
int optimalLocScore(int distFromHunt, int nNextAdj, int possHunt, int numHunt,
                    int numPossHunt, int locScore, int tuneScore, int distFromSea, int distAdjust, int loop) {
    int optimalScore;
    ...
    optimalScore = distFromHuntScore + 16 * nNextAdj - 4 * possHunt - numHunt - 2 * numPossHunt + locScore + tuneScore - distFromSeaScore - 16 * loop;
...
int numOfPossHunter(DracView gameState, Map map, Map railMap, LocationID dest, int *knownLoc,
    ...
    if (knownNumLoc > 0) {
        for (hunter = PLAYER_LORD_GODALMING; hunter < NUM_HUNTERS; hunter++) {
            if (dest == predictHunterMove(gameState, map, railMap, hunter, currRound, hHealth, trail, knownLoc, knownNumLoc, hTrail, path) || dest == hTrail[hunter][0]) count++;
        }
    }
    return count;
}
...
int isLoop(LocationID dest, LocationID trail[TRAIL_SIZE], LocationID hideTrail[TRAIL_SIZE]) {
    ...
    copyArray(hideTrail, tmpHideTrail, TRAIL_SIZE);
    locToDBOrHI(locAbbrev, dest, tmpTrail, tmpHideTrail);
    nextAdjLoc = whereCanIgo(&numLoc, tmpTrail,
                tmpHideTrail, dest, TRUE, TRUE);
    
    for (i = 0; i < numLoc; i++) {
        shiftLeft(tmpTrail, 0, TRAIL_SIZE - 1);
        shiftLeft(tmpHideTrail, 0, TRAIL_SIZE - 1);
        idToAbbrev(nextAdjLoc[i], locAbbrev);
        locToDBOrHI(locAbbrev, nextAdjLoc[i], tmpTrail, tmpHideTrail);
        if (tmpHideTrail[0] >= HIDE) count++;
    }
    free(locAbbrev);
    if (count == numLoc) return 1;
    else return 0;
}
...
int isSurrounded(Map map, Map railMap, LocationID *adjLoc, int numLoc, Round currRound,
    ...
    int i, count = 0;
    for (i = 0 ; i < numLoc ; i++) {
        if (distFromNearestHunter(map, railMap, adjLoc[i], path, currRound, hTrail) <= 1) count++;
    }
    if (numLoc - count <= 1) return 1;
    return 0;
}
...
int shouldGoToCD(Map map, Map railMap, LocationID dCurrLoc, LocationID *adjLoc, int numLoc, Round currRound, LocationID *path, LocationID hTrail[NUM_HUNTERS][TRAIL_SIZE], int distCurrFromHunt, LocationID trail[TRAIL_SIZE], LocationID hideTrail[TRAIL_SIZE]) {
    ...
    if ((inArray(trail, KLAUSENBURG, TRAIL_SIZE - 1) != -1) &&
        (inArray(trail, GALATZ, TRAIL_SIZE - 1) != -1)) return 0;
...
int isDangerous(Map map, Map railMap, int *knownLoc, int knownNumLoc,
    ...
    if (isSurrounded(map, railMap, adjLoc, numLoc, currRound, hTrail, path)) return 1;
    //for (i = 0; i < numLoc; i++) {
...
LocationID escapeToSea(DracView gameState, Map map, Map railMap, LocationID *adjLoc, int numLoc,
    ...
    *minLength = NUM_MAP_LOCATIONS;
    ...
    int surrounded = 0;
    if (isSurrounded(map, railMap, adjLoc, numLoc, currRound, hTrail, path)) surrounded = 1;
    ...
    for (x = 0; x < numLoc; x++) {
        ...
        if (surrounded == 1 && needHiding != 0 && possHunt > 0) numPossHunt++;
        if (knownNumLoc > 0) {
            if (knownLoc[knownNumLoc - 1] < 3 && needHiding != 0) numPossHunt++;
        }
...
LocationID escapeToCD(DracView gameState, Map map, Map railMap, LocationID *adjLoc, int numLoc,
    ...
    *minLength = NUM_MAP_LOCATIONS;
    ...
    for (i = 0; i < numLoc; i++) {
        ...
        if (isLoop(adjLoc[i], trail, hideTrail)) lengthToCD += 2;
...
LocationID escapeToCity(DracView gameState, Map map, Map railMap, LocationID *adjLoc, int numLoc,
    ...
    for (x = 0; x < numLoc; x++) {
        ...
        if (isLoop(adjLoc[x], trail, hideTrail)) locScore -= 24;
...
LocationID escapeToEmptyArea(DracView gameState, Map map, Map railMap, LocationID *adjLoc, int numLoc,
    ...
    for (i = 0; i < numLoc; i++) {
        ...
        if (stayToLeaveTrap(gameState, map, railMap, adjLoc[i], numPossHunt, currRound, knownLoc, knownNumLoc, hTrail, hHealth, trail, path) == 1) numPossHunt = 0;
        if (isLoop(adjLoc[i], trail, hideTrail)) locScore -= 36;
...
LocationID stayInSea(DracView gameState, Map map, Map railMap, LocationID *adjLoc, int numLoc,
    ...
    if (idToType(trail[0]) == SEA) {
        ...
        for (i = 0; i < numLoc; i++) {
            ...
            if (isLoop(adjLoc[i], trail, hideTrail)) distFromHunt -= 2;
